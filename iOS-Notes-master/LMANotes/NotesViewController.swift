...
class NotesViewController: UIViewController, AddNote, UpdateNote {
    ...
    var selectedIndex = 0
    var notes: [Note] = [
        Note(title: "Basic Programming thing", content: "variables, contants, data types, array, loop, fuctions, etc..", date: "")
    ]
    ...
    override func viewWillAppear(_ animated: Bool) {
        let button1 = UIBarButtonItem.init(title: "Logout", style: .plain, target: self, action: #selector(logoutUser))
    ...
    func updateNote(_ forKey: String, _ forvalue: [String]) {
        if let note = self.data.value as? [[String: [String]]] {
            ...
            updatedNote[selectedIndex].removeAll()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let note = self.data.value as? [[String: [String]]] {
            if note.indices.contains(indexPath.row) {
                ...
                cell.contentLabel.text = note[indexPath.row][keysArray[0]]?[0] ?? ""
                cell.dateLabel.text = "Last modified Date:\n\(note[indexPath.row][keysArray[0]]?[1] ?? "")"
            }
        }
        return cell
    }
...
extension NotesViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let note = self.data.value as? [[String: [String]]] {
            let keysArray = Array(note[indexPath.row].keys)
            FirebaseNote = Note.init(title: keysArray[0], content: note[indexPath.row][keysArray[0]]?[0] ?? "", date: note[indexPath.row][keysArray[0]]?[1] ?? "")
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            if let note = self.data.value as? [[String: [String]]] {
                var updatedNote = note
                updatedNote.remove(at: indexPath.row)
                if let email = Auth.auth().currentUser?.email {
                    let emailArrAtt = email.components(separatedBy: "@")
                    if let emailArr = emailArrAtt.first?.components(separatedBy: ".") {
                        Database.database().reference().child(emailArr[0]).setValue(updatedNote)
