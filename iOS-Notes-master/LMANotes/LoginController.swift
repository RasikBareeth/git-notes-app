...
//        }
        ...
        if let createUserViewController = storyBoard.instantiateViewController(withIdentifier: "createUserVC") as? CreateUserViewController {
            if let navigator = navigationController {
                navigator.pushViewController(createUserViewController, animated: true)
            }
         //   self.present(createUserViewController, animated:true, completion:nil)
        }
    }
...
class LoginView: UIView {
    ...
    lazy var signUpAccountButton: UIButton = {
            let button = UIButton()
    ...
    func setupSubviews(_ isForLoginPage: Bool = true) {
    ...
    private func setupSignUpAccountButton() {
        addSubview(signUpAccountButton)
        signUpAccountButton.translatesAutoresizingMaskIntoConstraints = false
        NSLayoutConstraint.activate([
            signUpAccountButton.leadingAnchor.constraint(
                equalTo: safeAreaLayoutGuide.leadingAnchor,
                constant: 15
            ),
            signUpAccountButton.trailingAnchor.constraint(
                equalTo: safeAreaLayoutGuide.trailingAnchor,
                constant: -15
            ),
            signUpAccountButton.heightAnchor.constraint(equalToConstant: 45),
            signUpAccountButton.centerYAnchor.constraint(equalTo: centerYAnchor, constant: 5),
        ])
    }
